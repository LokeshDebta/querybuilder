...
func connectDB() {
	...
	// If you were using Planetscale or db4free.net, update the host here
...
// connectDBWithCredentials connects to MySQL with provided credentials
func connectDBWithCredentials(username, password string) (*sql.DB, error) {
	dsn := fmt.Sprintf("%s:%s@tcp(db4free.net:3306)/querybuilder", username, password) // Update host
	db, err := sql.Open("mysql", dsn)
	if err != nil {
		return nil, fmt.Errorf("failed to connect to MySQL: %v", err)
	}
...
func createMySQLUser(username, password string, privileges []string) error {
	...
	for _, privilege := range privileges {
		privilegeQuery := fmt.Sprintf("GRANT %s ON *.* TO '%s'@'%'", privilege, username)
		_, err = db.Exec(privilegeQuery)
		if err != nil {
			return fmt.Errorf("failed to grant privilege %s: %v", privilege, err)
		}
...
func main() {
	// You can replace these with environment variables when deploying to Render
	connectDB()
	defer db.Close()
